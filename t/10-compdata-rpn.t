...
$res = rpn_calc([5], ' "ab\\"cde\\\'fgh\\\\ijk\\lmn\\\\\\o\\p" ',
    undef, {}, {}, 'example.org');
ok($res eq 'ab"cde\'fgh\\ijklmn\\op',
...
$res = rpn_calc([5], ' $not_present ', undef, $val_set, $results_test,
    'example.org');
ok(!defined($res),
    "associated-value tokens push undef when value doesn't exist");
...
$res = rpn_calc(['testTESTabc'], '"[a-z]([A-Z]+)([a-z]+)$" match',
    undef, {}, {}, 'example.org');
...
ok($res == 6,
    'exch is a no-op when there are fewer than two elements on the stack');
...
ok($res eq 'a',
    'index fetches elements, indexed from the top of the stack (1a)');

$res = rpn_calc(['c'], '"b" "a" 1 index pop', undef, {}, {}, 'example.org');
ok($res eq 'a',
    'index fetches elements, indexed from the top of the stack (1b)');

$res = rpn_calc(['c'], '"b" "a" 2 index', undef, {}, {}, 'example.org');
ok($res eq 'b',
    'index fetches elements, indexed from the top of the stack (2a)');

$res = rpn_calc(['c'], '"b" "a" 2 index pop', undef, {}, {}, 'example.org');
ok($res eq 'a',
    'index fetches elements, indexed from the top of the stack (2b)');

$res = rpn_calc(['c'], '"b" "a" 3 index', undef, {}, {}, 'example.org');
ok($res eq 'c',
    'index fetches elements, indexed from the top of the stack (3a)');

$res = rpn_calc(['c'], '"b" "a" 3 index pop', undef, {}, {}, 'example.org');
ok($res eq 'a',
    'index fetches elements, indexed from the top of the stack (3b)');
