...
sub check_response
{
    ...
    ok(defined($actual_response),
        "request $request_num: we got back a response");
    ok(
        !defined($actual_response->{'error'})
          && !defined($actual_response->{'error_text'}),
        "request $request_num: didn't get an error message"
    );
    ok(defined($actual_response->{'results'}),
        "request $request_num: got results in the response");

    cmp_deeply($actual_response->{'results'},
        $expected_results, "request $request_num: got the correct results");
}
...
sub error_expected
{
    ...
    ok(defined($response->{'error'}),
        "request $request_num: got an error response");
    ok(!defined($response->{'results'}),
        "request $request_num: didn't get results in the response");
}
...
my $response = $client->test1(node => 'a.example.net');
...
$response = $client->test1(node => []);

error_expected(3, $response);

# Request 4: simple test of rates
$response = $client->test2(node => ['a.example.net']);
...
$response = $client->test2(node => ['a.example.net', 'c.example.net_1']);
...
$response = $client->test3(node => 'b.example.net');
...
$response = $client->test4(
    node => ['a.example.net', 'c.example.net_1',
        'c.example.net_2']    # we expect nothing for c.example.net_1
);
...
$response = $client->test5(node => ['a.example.net', 'b.example.net'],);
...
$response =
  $client->test6(node => ['a.example.net', 'b.example.net', 'c.example.net_1'],
  );
...
$response = $client->test7(node => 'a.example.net',);
...
$response = $client->test8(node => 'b.example.net');
...
# see in production
$response = $client->interface(node =>
      ['a.example.net', 'b.example.net', 'c.example.net_1', 'c.example.net_2']);
...
$response = $client->test7(node => 'd.example.net',);
...
$response = $client->test9(
    node => [
        ...
        'c.example.net_1', 'c.example.net_2',
        'd.example.net'
    ]
);
