...
#    }
    ...
    if (defined $status_dir && -d $status_dir)
    {
        if (substr($status_dir, -1) ne '/')
        {
            $status_dir .= '/';
            $self->logger->debug(
                "The path for status_dir didn't include a trailing slash, added it: $status_dir"
            );
        }

        $self->_set_status_dir($status_dir);
        $self->logger->debug(
            "Found poller_status dir in config, using: " . $self->status_dir);
    ...
    {
        $self->logger->debug(
            "No valid poller_status dir defined in config, using: "
              . $self->status_dir);
    }
...
sub _get_config_objects
{
    ...
    $self->logger->debug("Got a total of "
          . scalar(keys %config_objects)
          . " $target_obj objects");
...
sub _process_hosts_config
{
    ...
    my $hosts =
      $self->_get_config_objects('/config/host', $self->hosts_dir,
    ...
    for my $host_name (keys %$hosts)
    {
        ...
        {
            $self->logger->error(
                "Could not find or create dir for monitoring data: $mon_dir");
        }
        else
        {
            $self->logger->debug(
                "Found or created status dir for $host_name successfully");
        }
    }
    ...
    for my $host_dir (glob($self->status_dir . '*'))
    {
        ...
        unless (exists $hosts->{$dir_name})
        {
            ...
            {
                $self->logger->error(
                    "Attempted to remove $host_dir, but failed!");
            }
            else
            {
        ...
        # The hosts' status dir exists,
        # make sure there are only status files for active groups
        else
        {
            for my $status_file (glob($host_dir . '/*'))
            {
                ...
                # Unless the host is configured for the group,
                # delete the status file
                unless (exists $hosts->{$dir_name}{group}{$1})
                {
                    unlink $status_file;
                    $self->logger->debug(
                        "Removed status file $file in $host_dir");
                }
            }
        }
    }
...
sub _process_groups_config
{
    ...
    my $groups =
      $self->_get_config_objects('/group', $self->groups_dir,
    ...
    for my $group_name (keys %$groups)
    {
        ...
        unless ($group->{retention})
        {
            ...
            $self->logger->debug(
                "Retention Period: $group->{retention} (default)");
        }
        else
        {
            $self->logger->debug(
                "Retention Period: $group->{retention} (specified)");
        }
...
# Set the packet results size (max_repetitions) or default to size from poller config
        unless ($group->{request_size})
        {
            ...
            $self->logger->debug(
                "Request Size: $group->{request_size} results (default)\n\n");
        }
        else
        {
            $self->logger->debug(
                "Request Size: $group->{request_size} results (specified)\n\n");
        }

        # Set the hosts that belong to the polling group
        $group->{hosts} = ();
        for my $host_name (keys %{$self->hosts})
        {
            my $host = $self->hosts->{$host_name};

            for my $host_group (keys %{$self->hosts->{$host_name}->{group}})
            {
...
sub start
{
    ...
    if ($self->daemonize)
    {
        ...
        if (defined($group_name))
        {
            ...
            $! = 0;
            setgid($gid);
            $self->_log_err_then_exit("Unable to set GID to $gid ($group_name)")
              if $! != 0;
        }
        ...
        if (defined($user_name))
        {
            ...
            $self->_log_err_then_exit("Unable to get UID for user '$user_name'")
              if !defined($uid);

            $! = 0;
            setuid($uid);
            $self->_log_err_then_exit("Unable to set UID to $uid ($user_name)")
              if $! != 0;
        }
...
sub _create_workers
{
    ...
    for my $group_name (keys %{$self->groups})
    {
        ...
        $self->logger->info(
            "Creating $group->{workers} child processes for group: $group_name"
        );
        ...
        $forker->run_on_finish(
            sub {
                my ($pid) = @_;
                $self->logger->error(
                    "Child worker process $pid ($group_name) has died.");
            }
        );
        ...
        for (my $worker_id = 0; $worker_id < $group->{workers}; $worker_id++)
        {
            ...
            {
                $self->logger->debug(
                    "Child worker process $pid ($group_name) created.");
