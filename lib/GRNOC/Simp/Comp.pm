    ...
    if ($self->daemonize)
    {
        ...
        if (!$pid)
        {
            ...
            if (defined($group_name))
            {
                ...
                $! = 0;
                setgid($gid);
                $self->_log_and_exit("Unable to set GID to $gid ($group_name)")
                  if $! != 0;
            }
            ...
            if (defined($user_name))
            {
                ...
                $self->_log_and_exit("Unable to get UID for user '$user_name'")
                  if !defined($uid);
                $! = 0;
                setuid($uid);
                $self->_log_and_exit("Unable to set UID to $uid ($user_name)")
                  if $! != 0;
            }
