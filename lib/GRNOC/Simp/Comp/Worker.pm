...
sub _convert_data {
    ...
    for my $host (keys %{$request->{'data'}}) {
        ...
        last unless ($conversions);

        # Apply each function included in conversions
        for my $conversion (@$conversions) {

            # Get the function definition and create the anonymous sub
            #my $func = eval "sub { my \$data = shift; $conversion->{definition} }";
            #if (!$func or $@) {
            #    $self->logger->error("Error while applying conversion function: $@");
            #}
            ...
            for my $data (@{$request->{'data'}{$host}}) {
                ...
                for my $meta (keys %{$request->{meta}}) {
                    if (exists $data->{$meta}) {
                        $data->{$request->{meta}{$meta}} = delete $data->{$meta};
                    }
                }
            }
        }
