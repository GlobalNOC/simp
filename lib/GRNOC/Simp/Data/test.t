...
my $grnoc_log   = GRNOC::Log->new( config => "/etc/grnoc/logs/log_unit_test.conf", watch => 120 );
...
my $response = $object->_is_oid_prefix($info->{'object'}{'prefix'},$info->{'object'}{'suffix'});
ok(defined($response) , "<< Testing is_ois_prefix. Response=> $response");

my %input = (	host 		=> $info->{'testFindKeys'}{'host'},
